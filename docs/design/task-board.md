...
- R4: CLI accepts an ordered board from exactly one source — positional `TASKS...` strings, or repeated `--task-file` paths where each whole `.md` file is one task — plus `window` (0–25, default 10), `context-mode` (`windowed-summaries` | `isolated`, default `windowed-summaries`), `summary-mode` (`truncate-tail` | `head-tail`, default `truncate-tail`), `summary-max-chars` (100–8000, default 1200), `stop-on-error` (default true), `retries-per-task` (0–3, default 1). Mixing the two board sources, or supplying neither, is a usage failure raised before admission.
...
- R6: Summarization is always applied on a shared board; raw prior results never enter the next prompt unsummarized. `truncate-tail` keeps a prefix plus `...[truncated N chars]`; `head-tail` keeps head+`...`+tail split evenly.
- R7: `context-mode=isolated` forwards nothing at all: the prompt carries no prior-results section, and `window`, `summary-mode`, and `summary-max-chars` are overridden rather than combined. This is the mode for a board of independently decomposed tasks.
- R8: Every `click` command and option in the repository documents itself in at least four substantial sentences, and every positional argument is named in its command's help text. This is enforced by rule C001 in a new `lint/` suite, not by review.
- R9: Command follows PR #32 gateway order and passes verified grant plus loaded SDK into a network-free runner.
- R10: `runtime list` surfaces the new capability automatically via catalog projection; CLI `_ALLOWED_PRICES` admits it at 2 cents.
...
- `TaskBoardLimit(IntEnum)`: `TURN_TIMEOUT_SECONDS=5*24*60*60`, `MAX_TASKS=500`, `MAX_TASK_CHARS=20_000`, `MAX_WINDOW=25`. One board task is a whole unit of agent work rather than a chat turn, so the timeout is a five-day ceiling on a wedged child process, not a budget a caller is expected to approach. The character bound matches the 20,000 that every other local runtime task already uses, because a whole `.md` task file routinely exceeds 4,000.
...
- `TASKS...` arguments, or repeated `--task-file PATH` where each whole `.md` file is one task. The two are mutually exclusive: `TaskBoardInputInvalid` covers both a mixed and an empty invocation, and `TaskBoardTaskFileInvalid` covers a non-`.md`, unreadable, or blank file. A file's own line breaks are task text and are never task separators.
- `--host` (auto|codex, default codex), `--window`, `--context-mode`, `--summary-mode`, `--summary-max-chars`, `--stop-on-error/--no-stop-on-error`, `--retries-per-task`, `--idempotency-key`.
- Every one of those, and the command itself, carries at least four substantial sentences of `help=`, held in a module-level `_<NAME>_HELP` constant so the prose is reviewable as prose.
...
- CREATE `lint/` — the repository's own lint suite: `run.py`, `baseline.json`, `README.md`, `core/` (6 modules), and `rules/c001_command_help_description_depth.py`.
- MODIFY `src/vidbyte_cli/lib/errors/failures.py` — board input failures.
- MODIFY `scripts/run_ci.py` — lint gate, task-board gate, wheel prompt check.
- MODIFY `AGENTS.md` — `lint/` in the Map, both commands in the Command Deck.
...
- Q4: `lint/` currently holds one rule. The next candidates are the module-docstring shape and the `no function alone in a module` contract from the field guide; neither is written yet because neither has been measured.
- Q5: C001's allowance starts at 58, which is every command and option that predates the gate. Lowering it is a separate change per command family, not part of this PR.
