...
- Accept the original 13 optional context inputs plus review-requested mistake, prohibition,
  approach, outcome, blocker, hypothesis, risk, and parent-trajectory context. Every context
  value is optional and repeatable except the original single-path inputs.
...
5. Context inputs: read only explicitly supplied files; UTF-8 text/Markdown + documented JSON schemas; reject missing files, directories, malformed JSON, unsupported schema versions; stable per-run context refs; preserve source identity (caller-supplied vs artifact-observed); per-file + total limits before model calls; explicit omission/truncation reporting; file contents are data, never instruction overrides; surface contradictions; never infer execution permission. Mistakes carry causes and lessons, forbidden actions are hard cuts copied into handoffs, approaches retain worked/failed/rejected/untried status, outcomes close prior-suggestion feedback, blockers name their wait condition, hypotheses carry confidence, risks require mitigation, and trajectory preserves the parent agent's supplied path.
...
8. Every idea carries one primary + optional secondary categories, horizon, relationship (`direct|alternative|adjacent|exploratory`), readiness (`ready|needs_evidence|needs_decision|blocked`), evidence refs limited to supplied IDs, assumptions/dependencies, a first action plus ordered suggested actions, decision points, considerations, completion criteria, review summary, and a full handoff.
9. The final handoff is action-centered: title and summary, suggested actions, decisions along the way, considerations, embedded evidence content plus refs and sources, completion checks, dependencies, warnings, and exact forbidden actions. `authority` defaults to `not_granted_by_this_handoff`; `stop_conditions` include completion, contradiction, missing authority, and missing prerequisites; `execution_prompt` renders deterministically from those validated fields.
...
- `__init__.py` registers the `agents` group; the `suggestion/` subpackage owns the `suggest` subgroup, its `run`, `categories`, and `handoff` commands, its request builder, renderer, and Markdown help library. The run command accepts all flags from §§4–5 of the thread and validates counts, categories, and input exclusivity before any model/provider work. Every help asset contains three to four prose paragraphs about agent behavior without embedding command syntax.
...
- `sdk.py`: only module importing `vidbyte-sdk`, lazily inside methods; accepts frozen, strictly validated local dataclass inputs; maps CLI provider/model values into SDK dataclasses; constructs generator/critic settings with independent SDK `ContextManager` instances and the custom context primitive placed after the system prompt; exposes provider/schema error classification.
...
- `categories.py`: loads and validates 17 detailed Markdown category definitions, each with Description, Goal, Intent, Timeline, Checklist, and Cautions sections; stable IDs drive CLI selection, prompt text, and schema validation.
