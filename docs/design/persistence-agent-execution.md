...
**Status:** Review revision implemented; full local gate passed
**Author:** Codex
**Created:** 2026-09-06
...
PR #26 comments 3951886729, 3951907427, 3951909703, 3951912773, and
3951948491 supersede the process-adapter design below. Use the existing SDK
`CodexHarnessAgent.arun(CodexRunInput.text(...))` on one agent instance. Its saved
thread ID resumes the same conversation; the CLI rejects missing or changed IDs,
non-completed results, and missing final text. An asyncio timeout bounds each turn
and cancellation unwinds the SDK client. No SDK change is needed.

Policy, time, and signature checks return frozen `RuntimeAdmissionCheck` dataclasses
with an `AdmissionReason` enum, including an explicit passed value. Preserve existing
public verdict reason strings and all admission-denial behavior.

Add `lib/constants/` for runtime reason enums, Codex provider configuration, timeout,
and progress messages. Progress describes preparation, credential resolution, paid
admission, receipt verification, initial execution, continuation phases, and final
delivery. Phase messages describe requests to improve the task; they do not claim
the agent performed a particular check. Never display continuation indices.

Additional files required by these comments: `lib/constants/__init__.py`,
`lib/constants/runtime.py`, `lib/constants/README.md`, and the packaged
`lib/runtime_primitives/persistence_system.md` prompt required by SDK construction.
Pin the SDK Codex dependency to verified commit
`6f9f84a257036e65bab9595d3c34e8a98cdcd96e`; do not rely on the older published 0.1.0.
Backend activation and verification already exist in commit `d2736523`.

Extend the existing admission script with SDK-boundary tests for all six strengths,
exact whitespace/Unicode task input, same-thread continuity, failure and timeout
short-circuiting, cancellation, progress channels, and denial before any SDK turn.
Exercise the actual SDK facade against a fake transport to catch translation drift.
The full `python scripts/run_ci.py` gate includes installed SDK and prompt checks.
No live model invocation or wallet debit is needed for this verification.

Validation: `python scripts/run_ci.py` passed on Windows/Python 3.11, including
26 admission/persistence tests, strict typing with SDK source analysis, all existing
CLI diagnostics, distribution metadata, both packaged prompts, a fresh wheel install,
and installed SDK imports. The SDK's environment merge requires empty overrides
for unrelated credential variables, otherwise parent secrets are inherited again.
No `lint/` suite exists in this repository, so no linter rule was added.

## Original implementation record

The sections below record the earlier subprocess implementation and backend work.
The review revision contract above is the current execution design.
