...
- Generate roles at runtime from a planner agent instead of shipping predetermined roles with
  ...
  `identity`, `personality`, `expertise`, `knowledge`, `skills`, and `goal` sections (review
  comments 3941891486 and 3942890348).
...
6. Each proposal fork is bounded by the fixed `role_timeout_seconds` default (300 seconds)
   on `EnsembleInputs`. There is no CLI surface for it: the bound is a run-safety constant,
   not a caller choice.
...
8. Stage one must run exactly one turn on a fresh Codex thread using the authored planner
   ...
   carrying roster-level analysis plus `name`, `identity`, `personality`, `expertise`,
   `knowledge`, `skills`, and `goal` for every role.
9. Each generated role's six sections must be assembled into that fork's system prompt, wrapped
...
The command layer shrinks to parsing, admission, verification, and rendering.
`SameHostEnsembleCommand` builds an `EnsembleInputs` model, asks `RuntimeLaunchPlanner` for
a `RuntimeLaunchPlan` exactly as `adversarial-team` does, loads the SDK, buys one admission
under the caller's idempotency key, and runs the layered `RuntimeAdmissionGate` over the
grant — following the verification-gateway shape PR #26 established for `persistence` — so
no fork ever starts unverified. Only then does it call
`EnsembleRunner.run(plan, inputs, sdk, grant)` directly. It does not go
...
[CLI] -> EnsembleInputs (validated) -> RuntimeLaunchPlan (host + cwd)
                 ...
                 (each: identity/personality/expertise/knowledge/skills/goal)
                                          |
             +----------------------------+----------------------------+
             |                            |                            |
...
2. `<instructions-and-output>` names the exact JSON keys the stage must return. This is not
   ...
   not at all.
3. `role_system.md` is the template the planner's six generated sections are substituted into.
...
Parses options into `EnsembleInputs`, builds the launch plan, loads the SDK, buys and
verifies one admission, calls the runner, and renders the result. Every trace of
caller-supplied roles is removed.
...
1. `SameHostEnsembleCommand.execute` validates the idempotency key, builds the plan, checks
   the host, and loads the SDK — all free — then requests admission through
   `admit_same_host_ensemble` and runs `RuntimeAdmissionGate.verify` over the grant before
   calling the runner. A rejected grant raises `RuntimeAdmissionNotVerified` with only the
   safe reason category, so no fork ever starts unverified.
2. `EnsembleRunner.run(plan, inputs, sdk, grant)` builds the stages from the verified inputs
   and runs the service on `grant.charged_cents`. The runner never touches the network.
