Extra compute runs one independent suggestion agent for each selected category and combines their structured results before critique. Each category receives the same goal and caller context, but its own focused category window, so the run can explore more distinct directions.
