**Forbidden Actions**
Forbidden Actions is the named prohibited action value that this command accepts for one suggestion workflow.
It gives the parent agent a stable way to state an action that must not be proposed or taken for this goal.
...
**Purpose of Forbidden Actions**
The purpose of Forbidden Actions is to make an action that must not be proposed or taken for this goal explicit before candidate generation starts.
...
**When not to use Forbidden Actions**
Do not provide Forbidden Actions merely because the option exists or because an empty value would look complete.
...
**Defaults and precedence for Forbidden Actions**
When it is omitted, the default is no supplied prohibited action..
...
**Forbidden Actions output contract**
Forbidden Actions is not printed as a separate progress message because stdout is reserved for the command result.
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --forbidden "{prohibited action}"
```

**Examples for Forbidden Actions**
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --forbidden "{prohibited action}"
vidbyte-cli --json agents suggest run --goal "{goal}" --forbidden "{value}"
```

**Related commands for Forbidden Actions**
...
**Failure modes for Forbidden Actions**
...
**Authentication and permissions for Forbidden Actions**
