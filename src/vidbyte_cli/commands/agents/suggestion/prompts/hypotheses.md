**Hypotheses**
Hypotheses is the named testable belief value that this command accepts for one suggestion workflow.
It gives the parent agent a stable way to state a belief about the goal or current state that may need evidence.
...
The purpose of Hypotheses is to make a belief about the goal or current state that may need evidence explicit before candidate generation starts.
...
Do not provide Hypotheses merely because the option exists or because an empty value would look complete.
...
**Defaults and precedence for Hypotheses**
When it is omitted, the default is no supplied testable belief..
...
**Hypotheses output contract**
Hypotheses is not printed as a separate progress message because stdout is reserved for the command result.
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --hypotheses "{testable belief}"
```
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --hypotheses "{testable belief}"
vidbyte-cli --json agents suggest run --goal "{goal}" --hypotheses "{value}"
```

**Related commands for Hypotheses**
...
**Authentication and permissions for Hypotheses**
