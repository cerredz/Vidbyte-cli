**Open Questions**
Open Questions is the named unresolved question value that this command accepts for one suggestion workflow.
It gives the parent agent a stable way to state an uncertainty that a useful next action should answer or narrow.
...
**Purpose of Open Questions**
The purpose of Open Questions is to make an uncertainty that a useful next action should answer or narrow explicit before candidate generation starts.
...
**When not to use Open Questions**
Do not provide Open Questions merely because the option exists or because an empty value would look complete.
...
**Defaults and precedence for Open Questions**
When it is omitted, the default is no supplied unresolved question..
...
Open Questions is not printed as a separate progress message because stdout is reserved for the command result.
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --question "{open question}"
```
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --question "{open question}"
vidbyte-cli --json agents suggest run --goal "{goal}" --question "{value}"
```

**Related commands for Open Questions**
...
**Failure modes for Open Questions**
...
**Authentication and permissions for Open Questions**
