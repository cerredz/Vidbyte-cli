**Decision Context**
Decision Context is the named open or constrained choice value that this command accepts for one suggestion workflow.
It gives the parent agent a stable way to state a decision that is pending, already bounded, or likely to shape the next move.
...
The purpose of Decision Context is to make a decision that is pending, already bounded, or likely to shape the next move explicit before candidate generation starts.
...
**When not to use Decision Context**
Do not provide Decision Context merely because the option exists or because an empty value would look complete.
...
**Defaults and precedence for Decision Context**
When it is omitted, the default is no supplied open or constrained choice..
...
**Decision Context output contract**
Decision Context is not printed as a separate progress message because stdout is reserved for the command result.
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --decision "{decision context}"
```

**Examples for Decision Context**
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --decision "{decision context}"
vidbyte-cli --json agents suggest run --goal "{goal}" --decision "{value}"
```

**Related commands for Decision Context**
...
**Failure modes for Decision Context**
...
**Authentication and permissions for Decision Context**
