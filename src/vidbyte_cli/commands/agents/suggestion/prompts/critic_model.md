**Critic Model**
Critic Model is the named critic model identifier value that this command accepts for one suggestion workflow.
It gives the parent agent a stable way to state the optional configured model identifier used only for independent review.
...
**Purpose of Critic Model**
The purpose of Critic Model is to make the optional configured model identifier used only for independent review explicit before candidate generation starts.
...
**When not to use Critic Model**
Do not provide Critic Model merely because the option exists or because an empty value would look complete.
...
**Critic Model inputs**
The value is one nonempty provider-supported model identifier.
...
**Defaults and precedence for Critic Model**
When it is omitted, the default is no supplied critic model identifier..
...
**Critic Model output contract**
Critic Model is not printed as a separate progress message because stdout is reserved for the command result.
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --critic-model "{critic-model-id}"
```

**Examples for Critic Model**
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --critic-model "{critic-model-id}"
vidbyte-cli --json agents suggest run --goal "{goal}" --critic-model "{value}"
```

**Related commands for Critic Model**
...
**Failure modes for Critic Model**
...
**Authentication and permissions for Critic Model**
