**Constraints**
Constraints is the named boundary value that this command accepts for one suggestion workflow.
It gives the parent agent a stable way to state a practical, technical, legal, resource, or timing boundary the idea must respect.
...
The purpose of Constraints is to make a practical, technical, legal, resource, or timing boundary the idea must respect explicit before candidate generation starts.
...
Do not provide Constraints merely because the option exists or because an empty value would look complete.
...
**Defaults and precedence for Constraints**
When it is omitted, the default is no supplied boundary..
...
Constraints is not printed as a separate progress message because stdout is reserved for the command result.
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --constraint "{boundary}"
```
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --constraint "{boundary}"
vidbyte-cli --json agents suggest run --goal "{goal}" --constraint "{value}"
```

**Related commands for Constraints**
...
**Authentication and permissions for Constraints**
