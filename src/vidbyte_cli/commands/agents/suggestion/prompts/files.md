**Supporting Files**
Supporting Files is the named caller-selected file value that this command accepts for one suggestion workflow.
It gives the parent agent a stable way to state a text, Markdown, or documented JSON file whose bounded content may inform the run.
...
**Purpose of Supporting Files**
The purpose of Supporting Files is to make a text, Markdown, or documented JSON file whose bounded content may inform the run explicit before candidate generation starts.
...
**When not to use Supporting Files**
Do not provide Supporting Files merely because the option exists or because an empty value would look complete.
...
**Supporting Files inputs**
The value is one existing .txt, .md, or .json path per repeated option.
...
**Defaults and precedence for Supporting Files**
When it is omitted, the default is no supplied caller-selected file..
...
**Supporting Files output contract**
Supporting Files is not printed as a separate progress message because stdout is reserved for the command result.
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --files "{path/to/context.md}"
```

**Examples for Supporting Files**
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --files "{path/to/context.md}"
vidbyte-cli --json agents suggest run --goal "{goal}" --files "{value}"
```

**Related commands for Supporting Files**
...
**Failure modes for Supporting Files**
...
**Authentication and permissions for Supporting Files**
