**Blockers**
Blockers is the named blocking condition value that this command accepts for one suggestion workflow.
It gives the parent agent a stable way to state a condition that currently prevents or delays useful progress.
...
**Purpose of Blockers**
The purpose of Blockers is to make a condition that currently prevents or delays useful progress explicit before candidate generation starts.
...
Do not provide Blockers merely because the option exists or because an empty value would look complete.
...
**Defaults and precedence for Blockers**
When it is omitted, the default is no supplied blocking condition..
...
Blockers is not printed as a separate progress message because stdout is reserved for the command result.
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --blockers "{blocking condition}"
```

**Examples for Blockers**
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --blockers "{blocking condition}"
vidbyte-cli --json agents suggest run --goal "{goal}" --blockers "{value}"
```

**Related commands for Blockers**
...
**Authentication and permissions for Blockers**
