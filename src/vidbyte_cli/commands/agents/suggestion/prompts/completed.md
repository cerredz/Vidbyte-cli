**Completed Work**
Completed Work is the named completed outcome value that this command accepts for one suggestion workflow.
It gives the parent agent a stable way to state outcomes that are already true and should not be proposed again.
...
**Purpose of Completed Work**
The purpose of Completed Work is to make outcomes that are already true and should not be proposed again explicit before candidate generation starts.
...
**When not to use Completed Work**
Do not provide Completed Work merely because the option exists or because an empty value would look complete.
...
**Defaults and precedence for Completed Work**
When it is omitted, the default is no supplied completed outcome..
...
**Completed Work output contract**
Completed Work is not printed as a separate progress message because stdout is reserved for the command result.
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --completed "{completed outcome}"
```
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --completed "{completed outcome}"
vidbyte-cli --json agents suggest run --goal "{goal}" --completed "{value}"
```

**Related commands for Completed Work**
...
**Failure modes for Completed Work**
...
**Authentication and permissions for Completed Work**
