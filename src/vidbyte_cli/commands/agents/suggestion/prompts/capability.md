**Capabilities**
Capabilities is the named available resource value that this command accepts for one suggestion workflow.
It gives the parent agent a stable way to state a skill, access grant, tool, person, or resource available to the caller.
...
The purpose of Capabilities is to make a skill, access grant, tool, person, or resource available to the caller explicit before candidate generation starts.
...
Do not provide Capabilities merely because the option exists or because an empty value would look complete.
...
**Defaults and precedence for Capabilities**
When it is omitted, the default is no supplied available resource..
...
Capabilities is not printed as a separate progress message because stdout is reserved for the command result.
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --capability "{available capability}"
```
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --capability "{available capability}"
vidbyte-cli --json agents suggest run --goal "{goal}" --capability "{value}"
```

**Related commands for Capabilities**
...
**Failure modes for Capabilities**
...
**Authentication and permissions for Capabilities**
