**General Context**
General Context is the named background facts that do not fit a narrower context type value that this command accepts for one suggestion workflow.
It gives the parent agent a stable way to state background facts that do not fit a narrower record type.
...
**Purpose of General Context**
The purpose of General Context is to make background facts that do not fit a narrower record type explicit before candidate generation starts.
...
**When not to use General Context**
Do not provide General Context merely because the option exists or because an empty value would look complete.
...
**General Context inputs**
The value is one nonempty text value per repeated option.
...
**Defaults and precedence for General Context**
When it is omitted, the default is no supplied background facts that do not fit a narrower context type..
...
**General Context output contract**
General Context is not printed as a separate progress message because stdout is reserved for the command result.
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --context "{background context}"
```

**Examples for General Context**
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --context "{background context}"
vidbyte-cli --json agents suggest run --goal "{goal}" --context "{value}"
```

**Related commands for General Context**
...
**Failure modes for General Context**
...
**Authentication and permissions for General Context**
