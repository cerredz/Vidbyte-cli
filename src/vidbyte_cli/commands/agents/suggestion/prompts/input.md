**Saved Result File**
Saved Result File is the named saved suggestion result path value that this command accepts for one suggestion workflow.
It gives the parent agent a stable way to state a previously emitted suggestions.result JSON document for local handoff extraction.
...
**Purpose of Saved Result File**
The purpose of Saved Result File is to make a previously emitted suggestions.result JSON document for local handoff extraction explicit before candidate generation starts.
...
**When not to use Saved Result File**
Do not provide Saved Result File merely because the option exists or because an empty value would look complete.
...
**Saved Result File inputs**
The value is one existing JSON path.
...
**Defaults and precedence for Saved Result File**
When it is omitted, the default is no supplied saved suggestion result path..
...
**Saved Result File output contract**
Saved Result File is not printed as a separate progress message because stdout is reserved for the command result.
...
**Examples for Saved Result File**
...
```text
vidbyte-cli agents suggest handoff --input "{result.json}" --idea "{idea-id}"
vidbyte-cli --json agents suggest run --goal "{goal}" --input "{value}"
```

**Related commands for Saved Result File**
...
**Failure modes for Saved Result File**
...
**Authentication and permissions for Saved Result File**
