**Directions to Avoid**
Directions to Avoid is the named rejected direction value that this command accepts for one suggestion workflow.
It gives the parent agent a stable way to state an approach or direction the caller does not want proposed.
...
**Purpose of Directions to Avoid**
The purpose of Directions to Avoid is to make an approach or direction the caller does not want proposed explicit before candidate generation starts.
...
**When not to use Directions to Avoid**
Do not provide Directions to Avoid merely because the option exists or because an empty value would look complete.
...
**Defaults and precedence for Directions to Avoid**
When it is omitted, the default is no supplied rejected direction..
...
**Directions to Avoid output contract**
Directions to Avoid is not printed as a separate progress message because stdout is reserved for the command result.
...
**How to use Directions to Avoid**
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --avoid "{direction to avoid}"
```

**Examples for Directions to Avoid**
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --avoid "{direction to avoid}"
vidbyte-cli --json agents suggest run --goal "{goal}" --avoid "{value}"
```

**Related commands for Directions to Avoid**
...
**Failure modes for Directions to Avoid**
...
**Authentication and permissions for Directions to Avoid**
