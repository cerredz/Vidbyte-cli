**Observed Outcomes**
Observed Outcomes is the named observed result value that this command accepts for one suggestion workflow.
It gives the parent agent a stable way to state what happened after earlier work, decisions, or suggestions.
...
**Purpose of Observed Outcomes**
The purpose of Observed Outcomes is to make what happened after earlier work, decisions, or suggestions explicit before candidate generation starts.
...
**When not to use Observed Outcomes**
Do not provide Observed Outcomes merely because the option exists or because an empty value would look complete.
...
**Defaults and precedence for Observed Outcomes**
When it is omitted, the default is no supplied observed result..
...
**Observed Outcomes output contract**
Observed Outcomes is not printed as a separate progress message because stdout is reserved for the command result.
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --outcomes "{observed result}"
```

**Examples for Observed Outcomes**
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --outcomes "{observed result}"
vidbyte-cli --json agents suggest run --goal "{goal}" --outcomes "{value}"
```

**Related commands for Observed Outcomes**
...
**Failure modes for Observed Outcomes**
...
**Authentication and permissions for Observed Outcomes**
