**Risks**
Risks is the named plausible exposure value that this command accepts for one suggestion workflow.
It gives the parent agent a stable way to state a plausible harm, failure mode, or exposure around the goal.
...
**Purpose of Risks**
The purpose of Risks is to make a plausible harm, failure mode, or exposure around the goal explicit before candidate generation starts.
...
Do not provide Risks merely because the option exists or because an empty value would look complete.
...
**Defaults and precedence for Risks**
When it is omitted, the default is no supplied plausible exposure..
...
**Risks output contract**
Risks is not printed as a separate progress message because stdout is reserved for the command result.
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --risks "{risk}"
```
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --risks "{risk}"
vidbyte-cli --json agents suggest run --goal "{goal}" --risks "{value}"
```

**Related commands for Risks**
...
**Authentication and permissions for Risks**
