**Approach History**
Approach History is the named previous direction value that this command accepts for one suggestion workflow.
It gives the parent agent a stable way to state a direction that was tried, considered, accepted, or rejected.
...
**Purpose of Approach History**
The purpose of Approach History is to make a direction that was tried, considered, accepted, or rejected explicit before candidate generation starts.
...
**When not to use Approach History**
Do not provide Approach History merely because the option exists or because an empty value would look complete.
...
**Defaults and precedence for Approach History**
When it is omitted, the default is no supplied previous direction..
...
**Approach History output contract**
Approach History is not printed as a separate progress message because stdout is reserved for the command result.
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --approaches "{previous direction}"
```

**Examples for Approach History**
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --approaches "{previous direction}"
vidbyte-cli --json agents suggest run --goal "{goal}" --approaches "{value}"
```

**Related commands for Approach History**
...
**Failure modes for Approach History**
...
**Authentication and permissions for Approach History**
