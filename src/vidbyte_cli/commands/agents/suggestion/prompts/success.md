**Successes**
Successes is the named proven pattern value that this command accepts for one suggestion workflow.
It gives the parent agent a stable way to state a method or outcome that has worked for the caller before.
...
**Purpose of Successes**
The purpose of Successes is to make a method or outcome that has worked for the caller before explicit before candidate generation starts.
...
**When not to use Successes**
Do not provide Successes merely because the option exists or because an empty value would look complete.
...
**Defaults and precedence for Successes**
When it is omitted, the default is no supplied proven pattern..
...
**Successes output contract**
Successes is not printed as a separate progress message because stdout is reserved for the command result.
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --success "{proven pattern}"
```

**Examples for Successes**
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --success "{proven pattern}"
vidbyte-cli --json agents suggest run --goal "{goal}" --success "{value}"
```

**Related commands for Successes**
...
**Authentication and permissions for Successes**
