**Parent Trajectory**
Parent Trajectory is the named larger effort value that this command accepts for one suggestion workflow.
It gives the parent agent a stable way to state the path and intent supplied by the parent agent or larger effort.
...
**Purpose of Parent Trajectory**
The purpose of Parent Trajectory is to make the path and intent supplied by the parent agent or larger effort explicit before candidate generation starts.
...
**When not to use Parent Trajectory**
Do not provide Parent Trajectory merely because the option exists or because an empty value would look complete.
...
**Defaults and precedence for Parent Trajectory**
When it is omitted, the default is no supplied larger effort..
...
**Parent Trajectory output contract**
Parent Trajectory is not printed as a separate progress message because stdout is reserved for the command result.
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --trajectory "{parent trajectory}"
```

**Examples for Parent Trajectory**
...
```text
vidbyte-cli agents suggest run --goal "{goal}" --trajectory "{parent trajectory}"
vidbyte-cli --json agents suggest run --goal "{goal}" --trajectory "{value}"
```

**Related commands for Parent Trajectory**
...
**Failure modes for Parent Trajectory**
...
**Authentication and permissions for Parent Trajectory**
