...
_STAGE_IMAGE_HELP = (
    "One image input for one stage, repeated once per stage in the same order as "
    "--stage-prompt. Give an https:// URL, an http:// URL, or a data: URL to send a remote "
    "image, and give anything else to send a local image file path, which is the right choice "
    "for a screenshot a review stage must look at. The value travels with that stage's turn "
    "only and never reaches another stage, so two stages that need the same image each need "
    "their own occurrence. Omit the option and every stage runs on text alone, which is the "
    "common case. A blank value is rejected before credentials are read or any admission is "
    "bought."
)
_STAGE_SKILL_HELP = (
    "One native Codex skill for one stage, repeated once per stage in the same order as "
    "--stage-prompt. Write each value as NAME=PATH, where NAME is the skill name and PATH is "
    "the skill directory the agent should load, splitting on the first equals sign so PATH "
    "itself may contain further equals signs. The skill is handed to that stage's agent only "
    "and never reaches another stage, so two stages that need the same skill each need their "
    "own occurrence. Omit the option and no stage loads a skill, which is the right default "
    "for stages that only need their prompts. A value without the separator, or with a blank "
    "name or path, is rejected before credentials are read or any admission is bought."
)
_STAGE_MENTION_HELP = (
    "One named resource mention for one stage, repeated once per stage in the same order as "
    "--stage-prompt. Write each value as NAME=PATH, where NAME is the resource name and PATH "
    "is the resource path the agent should read, splitting on the first equals sign so PATH "
    "itself may contain further equals signs. The mention is handed to that stage's agent only "
    "and never reaches another stage, so two stages that need the same resource each need "
    "their own occurrence. Omit the option and no stage receives a mention, which is the "
    "right default when the prompt already names everything. A value without the separator, "
    "or with a blank name or path, is rejected before credentials are read."
)
_STAGE_OUTPUT_SCHEMA_HELP = (
    "A JSON object schema string forcing one stage's agent to answer in a fixed shape, "
    "repeated once per stage in the same order as --stage-prompt. Use it when the next "
    'consumer is another stage: a stage forced to return {"files": [...], "summary": '
    '"..."} hands the next stage a handoff it can check mechanically instead of prose it '
    "may misread. The value must parse as a JSON object when given, and an empty omission "
    "means the stage answers in free prose, which stays the right choice for a final stage a "
    "person reads. A value that is not a JSON object is rejected before credentials are read "
    "or any admission is bought."
)
_STOP_ON_ERROR_HELP = (
    "What the run does the first time a stage exhausts its retries. With --stop-on-error, "
    "the default, the run halts immediately, keeps every result completed before the "
    "failure, and returns that prefix rather than running work that depends on a stage which "
    "never finished. With --no-stop-on-error the failed stage is recorded with a failed "
    "status, a short placeholder stands in for its text so stage indices stay aligned, and "
    "the next stage starts anyway. Keep the default for stages that build on each other, and "
    "turn it off for independent stages where one failure should not cancel the rest."
)
_RETRIES_PER_STAGE_HELP = (
    "How many extra attempts one stage receives after its first attempt fails, before the "
    "run treats that stage as failed. Every retry sends the identical prompt to a brand-new "
    "agent, so a retry recovers from a crashed or timed-out host rather than from a stage the "
    "agent understood but could not do. Each attempt is billed to your own OpenAI account, so "
    "a high retry count on a long run multiplies model cost even though the Vidbyte admission "
    "is charged once. Accepted values run from 0 to 3 with a default of 1, and a stage that "
    "succeeds on a retry produces exactly one result entry, never a duplicate."
)
_PARALLEL_HELP = (
...
_REFERENCE = (
    ...
    "  --stage-image           optional; one image URL or local path for that stage\n"
    "  --stage-skill           optional; one NAME=PATH native skill for that stage\n"
    "  --stage-mention         optional; one NAME=PATH resource mention for that stage\n"
    "  --stage-output-schema   optional; JSON object schema forcing that stage's shape\n"
    ...
    "--stop-on-error halts at the first exhausted stage; --max-retries-per-stage 0-3 "
    "(default 1) retries each stage on a fresh agent with backoff."
)
...
class StagesCommand:
    ...
    def _register_run(self, group: click.Group) -> None:
        ...
        @click.option("--stage-image", "images", multiple=True, help=_STAGE_IMAGE_HELP)
        @click.option("--stage-skill", "skills", multiple=True, help=_STAGE_SKILL_HELP)
        @click.option("--stage-mention", "mentions", multiple=True, help=_STAGE_MENTION_HELP)
        @click.option(
            "--stage-output-schema", "schemas", multiple=True, help=_STAGE_OUTPUT_SCHEMA_HELP
        )
        ...
        def _run(
            ...
            **stages: object,
            ...
            self.execute_run(ctx, task, parallel, key, stop_on_error, retries_per_stage, **stages)
    ...
    def execute_run(
        ...
        **stages: object,
        ...
        settings = self._settings(stages, parallel, stop_on_error, retries_per_stage)
        ...
        # prepare() records the launch paths and proves the SDK imports while the run is
        # still free; agents are built lazily per attempt so a failure never reuses one.
    ...
    def _settings(
        self, stages: dict[str, object], parallel: bool, stop_on_error: bool, retries: int
    ) -> StagesSettings:
        ...
        images = self._aligned(stages, "images", len(prompts))
        skills = self._aligned(stages, "skills", len(prompts))
        mentions = self._aligned(stages, "mentions", len(prompts))
        schemas = self._aligned(stages, "schemas", len(prompts))
        ...
        specs = [
            self._spec(
                ...
                images[index] if images else "",
                skills[index] if skills else "",
                mentions[index] if mentions else "",
                schemas[index] if schemas else "",
            )
