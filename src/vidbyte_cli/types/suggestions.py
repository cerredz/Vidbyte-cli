...
class SuggestionCriticReview(BaseModel):
    """Combined critic output: per-candidate verdicts with rubric plus whole-slate context."""

    model_config = ConfigDict(extra="forbid", frozen=True)
    critiques: tuple[SuggestionCritique, ...] = Field(
        max_length=30,
        description=(
            "One critique per reviewed candidate, each with verdict, signals, issues, "
            "and the ten-section rubric assessment. Preserved for traceability while "
            "the generator keeps ownership of the replacement slate."
        ),
    )
    context: SuggestionCriticContext = Field(
        description=(
            "One bounded whole-slate signal placed at end-of-conversation for the "
