...
class TaskBoardDecomposeInvalid(CliError):
    """Decomposition was combined with a feature that requires stable board indices."""

    code = CliErrorCode.INVALID_ARGUMENT
    exit_status = ExitCode.USAGE

    def __init__(self, conflict: str) -> None:
        # Names the conflicting option without repeating task text or other local input.
        super().__init__(
            f"--allow-decompose cannot be combined with {conflict}.",
            description=(
                "Decomposition replaces one task with subtasks at the same index, so every "
...
class LocalDocumentInvalid(CliError):
    """A local JSON document was readable but did not satisfy the caller's document model."""

    code = CliErrorCode.OPERATION_FAILED
    exit_status = ExitCode.OPERATIONAL_FAILURE

    def __init__(self, name: str, reason: str, cause: Exception | None = None) -> None:
        # Same fixed-category contract as LocalFileReadFailed: the reason is authored text chosen
        # by LocalDocumentStore, never a validator message that could quote document contents.
        super().__init__(
            f"Local document {name} is invalid: {reason}.",
            description=(
                "The file could be addressed, but its contents are not a document the command "
                f"can use because {reason}. Nothing was changed on disk, and the document was "
                "not replaced with an empty default. Retrying without repairing or removing the "
                "document will fail the same way."
            ),
            trace="LocalDocumentStore bounded, parsed, and validated the document against a model.",
            hint="Repair the document so it matches its schema, or remove it, then retry.",
            cause=cause,
        )
        self.reason = reason
...
class SuggestionProjectInvalid(CliError):
    """A project key, title, or description did not satisfy the local project contract."""
    ...
    def __init__(self, detail: str) -> None:
        ...
        super().__init__(
            ...
            description=(
                "A project key must be 1-64 lowercase letters, numbers, hyphens, or "
                "underscores beginning with a letter or number, and a new project also needs a "
                "nonempty title of at most 200 characters and description of at most 4000. The "
                "request was rejected before any project file was read or written. Existing "
                "projects and their feedback were not changed."
            ),
            trace="SuggestionProject validated the key and fields in its input dataclass.",
...
class SuggestionProjectExists(CliError):
    ...
    def __init__(self, key: str) -> None:
        ...
        super().__init__(
            ...
            trace="SuggestionProject found a duplicate key before its first write.",
...
class SuggestionProjectNotFound(CliError):
    ...
    def __init__(self, key: str) -> None:
        ...
        super().__init__(
            ...
            trace="SuggestionProject looked up an exact project key in projects.json.",
...
class SuggestionProjectStateUnreadable(CliError):
    ...
    def __init__(self, detail: str, cause: Exception | None = None) -> None:
        ...
        super().__init__(
            ...
            trace=f"SuggestionProject rejected local JSON state: {detail}.",
...
class SuggestionProjectWriteFailed(CliError):
    ...
    def __init__(self, cause: Exception) -> None:
        ...
        super().__init__(
            ...
            trace="SuggestionProject failed while replacing a local JSON document.",
...
class SuggestionFeedbackInvalid(CliError):
    ...
    def __init__(self, detail: str) -> None:
        ...
        super().__init__(
            ...
            description=(
                "Feedback requires an existing project and nonempty suggestion text of at most "
                "8192 characters, with an optional reason of the same bound. The request was "
                "rejected before the memory file changed. The CLI never infers acceptance or "
                "rejection from an ambiguous user response."
            ),
            trace="SuggestionFeedbackInput validated explicit feedback before persistence.",
            hint=(
