...
the factory wraps its manifest in a ManifestHarness and the generic dispatch/present/error
...
class ManifestHarness(BaseHarness):
    ...
    def commands(self, ctx: HarnessContext) -> list[HarnessCommandDef]:
        ...
        return [_spec_to_def(spec) for spec in self._manifest.commands]


def _spec_to_def(spec: HarnessCommandSpec) -> HarnessCommandDef:
