...
|---|---|---|
| C003 | markdown-xml-section-depth | Every paired XML section in a tracked Markdown prompt carries 6-8 complete sentences as prose, or 3-4 complete sentences per numbered step when the section is written as an enumerated procedure. |
...
| C006 | suggestion-help-from-assets | Every `help=` in `commands/agents/suggestion/` — group, command, flag, or option — is a module constant bound to `SuggestionHelpLibrary().load(...)`, or a conditional between two such constants, so suggestion help lives in its own Markdown asset (PR #84 review). |
