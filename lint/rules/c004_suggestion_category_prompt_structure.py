    ...
    def _analyze_file(self, source: SourceFile) -> list[Finding]:
        ...
        removed_line = next(
            (
                ...
                and match.group("text").strip().lower() in {"timeline", "checklist"}
            ),
            None,
        )
        if removed_line is not None:
            findings.append(
                self._finding(source, removed_line, "removed-section", "timeline/checklist")
            )
...
class SuggestionCategoryPromptStructureRule(Rule):
    ...
    severity = "blocking"
    summary = "Category prompts have a title, 6-8 sentence description, and 8-10 considerations."
    ...
    def explain(self, finding: Finding) -> Diagnostic:
        return Diagnostic(
            ...
            why_blocked=(
                ...
                "to consider, while removing the old timeline and checklist split."
            ),
            how_to_fix=(
                ...
                "and provide eight-to-ten specific bullet points. Remove `Timeline` and "
                "`Checklist` sections instead of padding them."
            ),
            correct_examples=(
