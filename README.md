...
Install Codex on PATH first. OpenAI credentials resolve from `OPENAI_API_KEY`, then the
...
Ctrl+C stops the active child process tree. A failed or hour-long turn stops the loop;
...
Only the final response is written to stdout; progress goes to stderr. JSON output includes
